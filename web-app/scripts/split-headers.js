...
const outDir = 'C:\\Projects\\Nobevra Web App Project\\web-app\\src\\components\\invoice\\templates\\footers';
const inputFile = 'C:\\Projects\\Nobevra Web App Project\\web-app\\src\\components\\invoice\\templates\\FooterRenderers.tsx';
...
// Write the separated files
for (const [prefix, stmts] of Object.entries(groups)) {
  const componentName = `${prefixMap[prefix]}Header`;
...
`.trim();
  ...
  fs.writeFileSync(path.join(outDir, `${prefixMap[prefix]}Headers.tsx`), fileContent);
}

// Write the Factory Dispatcher back to HeaderRenderers.tsx
const factoryContent = `
import React, { Suspense } from 'react';
import { SharedEngineProps } from './types';
...
console.log('Successfully refactored HeaderRenderers.tsx into ' + Object.keys(groups).length + ' modules!');
