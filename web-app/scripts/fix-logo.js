...
function fixLogo(file) {
    ...
    // <img src="/images/logo.png" alt="Nobevra Logo" className="h-10 w-auto" />
    content = content.replace(/<img[^>]*src="\/images\/logo.png"[^>]*\/>/g, (match) => {
        let className = match.match(/className="([^"]+)"/) ? match.match(/className="([^"]+)"/)[1] : "";
        return `<Image src="/images/logo.png" alt="Nobevra Logo" width={155} height={64} className="${className}" priority />`;
    });
...
fixLogo('c:/Projects/Nobevra Web App Project/web-app/src/components/shared/Navbar.tsx');
fixLogo('c:/Projects/Nobevra Web App Project/web-app/src/components/shared/Footer.tsx');
