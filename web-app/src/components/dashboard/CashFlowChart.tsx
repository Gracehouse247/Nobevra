...
export default function CashFlowChart({ invoices = [], currencyCode = 'USD', expenses: expenseList = [] }: { invoices?: Invoice[], currencyCode?: string, expenses?: any[] }) {
    ...
    const healthConfig = {
        healthy: { label: 'Healthy', icon: TrendingUp, color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-500/10', border: 'border-emerald-100 dark:border-emerald-500/20' },
        neutral: { label: 'Stable', icon: Minus, color: 'text-slate-500 dark:text-slate-300', bg: 'bg-slate-50 dark:bg-white/5', border: 'border-noble-card-border' },
        critical: { label: 'Review Cash Flow', icon: TrendingDown, color: 'text-red-500 dark:text-red-400', bg: 'bg-red-50 dark:bg-red-500/10', border: 'border-red-100 dark:border-red-500/20' },
    }[healthStatus];
