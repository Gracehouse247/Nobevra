...
export const ToggleRow = ({ label, description, icon, iconBg, checked = false, onChange, premium }: ToggleRowProps) => {
    return (
        <div
            ...
            tabIndex={0}
            className="flex items-center justify-between px-5 py-3.5 hover:bg-slate-50/70 cursor-pointer transition-colors"
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    <div className="flex items-center gap-1.5">
                        <p className="text-[13px] font-semibold text-slate-800 font-[Inter,sans-serif]">{label}</p>
                        {premium && <PremiumBadge tier={premium} iconOnly />}
                    </div>
                    <p className="text-[11px] text-slate-500 mt-0.5 font-[Inter,sans-serif]">{description}</p>
                </div>
            </div>
