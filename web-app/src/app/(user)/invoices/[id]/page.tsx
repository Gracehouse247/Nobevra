...
export default function InvoiceDetailView() {
    ...
    const getStatusConfig = (status: string) => {
        switch (status?.toLowerCase()) {
            ...
            case 'draft': return { color: 'text-slate-500', bg: 'bg-slate-50', border: 'border-slate-200', icon: Clock, label: 'DRAFT' };
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-8 font-inter">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <button 
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 bg-white text-slate-700 hover:bg-slate-50 rounded-xl text-xs font-bold transition-colors border border-slate-200"
                    >
                    ...
                    <button 
                        ...
                        className="flex items-center gap-1.5 px-4 py-2 bg-white text-slate-700 hover:bg-slate-50 rounded-xl text-xs font-bold transition-colors border border-slate-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {sendingEmail ? <Loader2 className="w-4 h-4 text-slate-400 animate-spin" /> : <Mail className="w-4 h-4 text-slate-400" />}
            ...
            {/* ── Invoice Document ─────────────────────────────────────────────── */}
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[32px] shadow-sm border border-slate-200/60 overflow-hidden">
                {/* Header Section */}
                <div className="p-8 md:p-12 border-b border-slate-100 relative overflow-hidden bg-slate-50/30">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 relative z-10">
                        <div>
                            <div className="w-16 h-16 bg-white border border-slate-200 rounded-2xl flex items-center justify-center text-slate-400 mb-6 shadow-sm">
                                <Building size={32} />
                            </div>
                            <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter mb-2">Invoice {invoice.invoice_number}</h1>
                            <div className="text-slate-500 font-medium">Billed to: <span className="font-bold text-slate-800">{client?.name || 'Unnamed Client'}</span></div>
                        </div>
                        <div className="text-left md:text-right">
                            ...
                            <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
                                ...
                                <div className="text-slate-900 font-black">{invoice.issue_date || invoice.created_at?.split('T')[0] || '—'}</div>
                ...
                <div className="p-8 md:p-12">
                    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">Line Items</h3>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {invoice.invoice_items?.map((item: any) => (
                                    <tr key={item.id} className="text-slate-700">
                                        <td className="py-4 pr-4 font-medium text-slate-900">{item.description || item.name}</td>
                                        ...
                                        <td className="py-4 pl-4 text-right tabular-nums font-bold text-slate-900">{formatCurrency(item.total_price)}</td>
                                    </tr>
                                ))}
                                {(!invoice.invoice_items || invoice.invoice_items.length === 0) && (
                                    <tr>
                                        <td colSpan={4} className="py-8 text-center text-slate-400 italic">No line items recorded.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
                ...
                {/* Summary Section */}
                <div className="p-8 md:p-12 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row gap-12 justify-between">
                    <div className="flex-1 max-w-sm">
                        {invoice.metadata?.bank_name && (
                            <div>
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Payment Instructions</h3>
                                <div className="bg-white p-5 rounded-2xl border border-slate-200/60 shadow-sm">
                                    <div className="space-y-2 text-sm">
                                        ...
                                        <div className="flex justify-between"><span className="text-slate-500">Account No.</span><span className="font-black text-[#0599D5] tracking-wider">{invoice.metadata.account_number}</span></div>
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="flex-1 md:max-w-md w-full">
                        <div className="space-y-4 bg-white p-6 rounded-2xl border border-slate-200/60 shadow-sm">
                            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                                <span className="text-slate-500 font-medium text-sm">Subtotal</span>
                                <span className="text-slate-900 font-bold tabular-nums">{formatCurrency(invoice.subtotal || 0)}</span>
                            </div>
                            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                                <span className="text-slate-500 font-medium text-sm">Tax ({invoice.tax_rate}%)</span>
                                <span className="text-slate-900 font-bold tabular-nums">{formatCurrency(invoice.tax_amount || 0)}</span>
                            </div>
                            {invoice.discount_amount > 0 && (
                                <div className="flex items-center justify-between pb-4 border-b border-slate-100 text-emerald-600">
