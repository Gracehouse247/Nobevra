    ...
    return (
        <div className="max-w-4xl mx-auto space-y-8">
            <div>
                ...
                <p className="text-slate-500 mt-1">White-label your invoices by serving them from your own domain.</p>
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="font-semibold text-slate-900">White-Label Domains</h2>
                            <p className="text-sm text-slate-500">Map a CNAME record to our servers.</p>
                        </div>
                    </div>
                    <button 
                        onClick={() => {
                ...
                <div className="p-0">
                    {domains.length === 0 ? (
                        <div className="p-8 text-center text-slate-500 text-sm">No custom domains configured.</div>
                    ) : (
                            ...
                            {domains.map(d => (
                                <div key={d.id} className="p-6 flex flex-col gap-4">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-3">
                                            <div className="font-bold text-slate-900 text-lg">{d.domain_name}</div>
                                            {d.status === 'active' ? (
                                    ...
                                    {d.status === 'pending' && (() => {
                                        ...
                                        <div className="bg-slate-50 rounded-lg p-5 border border-slate-200">
                                            <h4 className="font-bold text-slate-900 text-sm mb-3">DNS Configuration Required</h4>
                                            <p className="text-sm text-slate-600 mb-4">Please log in to your domain registrar (e.g. GoDaddy, Cloudflare, Namecheap) and add the following record:</p>
                                            
                                            <div className="grid grid-cols-3 gap-4">
                                                <div>
                                                    <div className="text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">Type</div>
                                                    <div className="bg-white border border-slate-200 rounded px-3 py-2 text-sm font-mono text-slate-700">{isRoot ? 'A' : 'CNAME'}</div>
                                                </div>
                                                <div>
                                                    <div className="text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">Name / Host</div>
                                                    <div className="bg-white border border-slate-200 rounded px-3 py-2 text-sm font-mono text-slate-700 flex justify-between">
                                                        {isRoot ? '@' : d.domain_name.split('.')[0]} 
                                                    </div>
                                                </div>
                                                <div>
                                                    <div className="text-xs font-bold text-slate-400 mb-1 uppercase tracking-wider">Value / Target</div>
                                                    <div className="bg-white border border-slate-200 rounded px-3 py-2 text-sm font-mono text-slate-700 flex justify-between">
