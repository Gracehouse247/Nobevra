...
## 1. PURPOSE AND SCOPE
This Data Retention & Deletion Policy outlines how long The Noble's Technology Service ("NobleInvoice," "we," "our") retains personal data, business data, and system logs, and the processes by which such data is securely deleted or anonymized.

This policy applies to all users of the NobleInvoice platform, across all our services (Web, Android, iOS, APIs). It forms part of our Global Compliance Framework and ensures our adherence to the Nigeria Data Protection Act (NDPA), GDPR, and other applicable global standards.
...
* **AI Usage Logs:** Inputs provided to the NobleInvoice AI Assistant (including receipt OCR inputs and prompt contexts) are retained strictly for auditing and abuse-prevention purposes and are permanently deleted after **1 year**.
...
Even if you request deletion, we may be required by law to retain certain specific records (such as billing history for your NobleInvoice subscription payments) for tax and accounting purposes (typically up to 7 years depending on jurisdiction). This data is isolated and access is strictly restricted.

## 5. LOCAL DATA ON MOBILE DEVICES
If you use the NobleInvoice mobile application (iOS or Android), some data (such as draft invoices) is stored locally on your device using encrypted secure storage. 
